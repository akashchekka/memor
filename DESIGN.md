...
This means intersecting two posting lists (the core trigram query operation) runs in microseconds even for thousands of entries.
...
| y | type | string | Yes | s=semantic, e=episodic, p=procedural, f=preference |
...
1. **One entry per line.** No multi-line entries.
...
### 9.1 Trigram Inverted Index (index/trigrams.bin)
...
- Store as **Roaring Bitmap** dump (~10x smaller than raw int lists, fast AND/OR).
...
- Instant "definitely not here" check before committing to a full scan.

### 9.4 Recency Ring (index/recency.json)

- Last 256 accessed ids, MRU first. Cheap freshness boost for ranking.
...
- Episodic entries older than archive_after_days (default 90) are archived.
...
memor import project-memories.jsonl
memor import project-memories.jsonl --dry-run
memor import project-memories.jsonl --trust low
memor knowledge export > knowledge-bundle.jsonl
memor knowledge import knowledge-bundle.jsonl
```
...
| **Token counting** | `tiktoken-go` | Accurate OpenAI-compatible token counting |
| **SHA256** | `crypto/sha256` | Stdlib, no external dependency |
| **Roaring Bitmaps** | `roaring-go` | Mature, production-grade, 10x smaller than raw int lists |
